...
//    }
    ...
    public void downloadFile(HttpServletResponse response,String type) throws IOException {
        ...
        try {
            response.setHeader("Content-Disposition", "attachment;filename="+ new String((type + ".xls").getBytes("gbk"), StandardCharsets.ISO_8859_1));//下载文件的名称
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        ServletOutputStream sout = response.getOutputStream();
        try (BufferedInputStream bis = new BufferedInputStream(f); BufferedOutputStream bos = new BufferedOutputStream(sout)) {
